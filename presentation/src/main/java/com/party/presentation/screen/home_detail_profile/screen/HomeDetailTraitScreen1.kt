...
fun HomeDetailTraitRoute1(
    ...
    HomeDetailTraitScreen1(
        snackBarHostState = snackBarHostState,
        onNavigationClick = { navController.popBackStack() },
        state = state,
        onAction = { action -> viewModel.onAction(action = action)},
        onGotoNext = { navController.navigate(route = Screens.Trait2)},
        onClose = { viewModel.onAction(action = HomeDetailProfileAction.OnShowFinishDialog(isShow = true))},
        onSkip = { navController.navigate(route = Screens.Trait2)}
    )
}

@Composable
private fun HomeDetailTraitScreen1(
    snackBarHostState: SnackbarHostState,
        ...
        Column(
            ...
            Column(
                ...
                val steps = listOf(
                    ...
                    StepInfo("3", "성향선택(1/4)", StepStatus.CURRENT)
                )
                ...
                Trait1Section(
                    modifier = Modifier
                        .weight(1f)
                    ,
                    selectedTraitList1 = state.selectedTraitList1,
                    traitList = state.personalityList.filter { it.id == 1 },
                    onSelect = { onAction(HomeDetailProfileAction.OnSelectTrait1(it))}
                )
            }

            DetailProfileNextButton(
                text = stringResource(id = R.string.common1),
                textColor = if(state.selectedTraitList1.isNotEmpty()) BLACK else GRAY400,
                containerColor = if(state.selectedTraitList1.isNotEmpty()) PRIMARY else LIGHT400,
                onClick = onGotoNext
            )
...
@Composable
private fun Trait1Section(
    modifier: Modifier,
    selectedTraitList1: List<PersonalityListOption>,
    traitList: List<PersonalityList>,
    onSelect: (PersonalityListOption) -> Unit,
) {
    // id가 1인 질문의 옵션들만 가져오기
    val trait1Options = traitList.find { it.id == 1 }?.personalityOptions ?: emptyList()
    ...
    LazyColumn(
        ...
        itemsIndexed(
            ...
            val isSelected = selectedTraitList1.any { it.id == option.id }
